...
import type { OrtsteilImportRow, OrtsteilDetails } from '@/models/Ortsteile'
import { fetchOrtsteile, createOrtsteil } from '@/services/svwsService'
import { fetchOrteKatalog } from '@/services/katalogService'
import type { OrtKatalogEintrag } from '@/models/ImportSchema'

export const useOrtsteileStore = defineStore('ortsteile', () => {
  const rows = ref<OrtsteilImportRow[]>([])
  const existingOrtsteile = ref<OrtsteilDetails[]>([])
  ...
  function setRows(newRows: OrtsteilImportRow[]): void {
    rows.value = newRows
  }

  function updateRow(id: string, patch: Partial<OrtsteilImportRow>): void {
  ...
  function clear(): void {
    rows.value = []
    existingOrtsteile.value = []
  }

  async function loadExisting(): Promise<{ error?: string }> {
    loadingExisting.value = true
    try {
      existingOrtsteile.value = await fetchOrtsteile()
      return {}
    } catch (e) {
      return { error: e instanceof Error ? e.message : 'Fehler beim Laden der Ortsteile' }
    } finally {
  ...
  async function uploadAll(selectedIds?: Set<string>): Promise<{ sent: number; failed: number }> {
    ...
    let orteMap: Map<string, OrtKatalogEintrag> | undefined
    try {
    ...
    for (let i = 0; i < updated.length; i++) {
      ...
      const result = await createOrtsteil(row, orteMap)
      if (result.success) {
